...
function PlayerProgress({ snapshot }: { snapshot: GameSnapshot }) {
  ...
  return <div className="panel player-progress">
    ...
    <div className="personal-economy-grid">
      <article><small>ЛИЧНЫЕ МОНЕТЫ</small><b>{snapshot.personalEconomy.coins.toLocaleString("ru-RU")}</b><span>кошелёк игрока</span></article>
      <article><small>ДОБЫЧА</small><b>{snapshot.personalEconomy.gatherCooldownSeconds > 0 ? `${Math.ceil(snapshot.personalEconomy.gatherCooldownSeconds / 60)} мин` : "готова"}</b><span>кулдаун 2 часа</span></article>
      <article><small>ИНСТРУМЕНТ</small><b>{snapshot.personalEconomy.toolTier ? `ур.${snapshot.personalEconomy.toolTier}` : "нет"}</b><span>{snapshot.personalEconomy.toolTier ? `${snapshot.personalEconomy.toolUsesLeft}/25 применений` : "!магазин"}</span></article>
      <article><small>ДОМ</small><b>ур.{snapshot.personalEconomy.homeLevel}</b><span>{snapshot.personalEconomy.homeHourlyCoins ? `+${snapshot.personalEconomy.homeHourlyCoins} монет/ч` : "не построен"}</span></article>
    </div>
    <div className="personal-inventory"><small>ЛИЧНЫЙ ИНВЕНТАРЬ</small><span>⚙️ {snapshot.personalEconomy.inventory.steel}</span><span>⛽ {snapshot.personalEconomy.inventory.fuel}</span><span>🌾 {snapshot.personalEconomy.inventory.food}</span><span>🔬 {snapshot.personalEconomy.inventory.tech}</span></div>
    {snapshot.personalEconomy.gatherBoostUntil && new Date(snapshot.personalEconomy.gatherBoostUntil).getTime() > Date.now() && <div className="admin-effect-pill">⚡ +50% к добыче до {new Date(snapshot.personalEconomy.gatherBoostUntil).toLocaleTimeString("ru-RU", {hour:"2-digit",minute:"2-digit"})}</div>}
...
function StateViewInner({ snapshot, onClaim, onPolitics, onGovernment, onCustomize }: Props) {
  ...
  return <div className={`state-screen game-scene state-theme-${snapshot.state.theme} ${snapshot.state.isFreeport ? "freeport-profile" : ""}`} style={{ ["--state-color" as any]: snapshot.state.color }}>
    ...
    {!snapshot.state.isFreeport && snapshot.state.economySleeping && <div className="economy-sleep-banner"><b>Инфраструктура в спящем режиме</b><span>Бонусы зданий к добыче заморожены. Первая сдача сырья через !сдать снова активирует их.</span></div>}
