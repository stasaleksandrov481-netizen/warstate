...
function StrategyPanelInner({ snapshot, onActivity, onSupport, onSurrender }: Props) {
  ...
  return (
    <div className="strategy-screen game-scene">
      <section className="strategy-hero strategy-hero-v2">
        <div className="strategy-hero-main">
          ...
          <div className="strategy-health-row">
            <span><i style={{ width: `${state.islandIntegrity}%` }} /><b>{state.islandIntegrity}%</b><small>прочность территории</small></span>
      ...
      {state.isBeginnerIsland && <div className="beginner-banner"><b>Учебный округ</b><span>Атаки запрещены · максимум ур. 5 · безопасные активности · поддержка только обороны.</span></div>}
