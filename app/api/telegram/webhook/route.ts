...
async function sendFreeportMessage(chatId: number) {
  return telegramApi("sendMessage", {
    ...
    text:
      `⚓ FREEPORT\n────────────\n` +
      `Нейтральная территория для свободных игроков.\n\n` +
      `• прокачивай профиль\n• находи государства на карте\n• вступай в их Telegram-чаты\n\n` +
      `После вступления выбери государство и нажми «Перейти» — бот проверит членство автоматически.`,
    reply_markup: {
...
async function sendLaunchMessage(chatId: number, title?: string) {
  ...
  return telegramApi("sendMessage", {
    ...
    text:
      ...
      `${title ? `«${title}»` : "Этот чат"} теперь участвует в общем мире WARSTATE: экономика, правительство, карта материка, дипломатия, выборы и войны.\n\n` +
      ...
      `🚨 Днём каждые 5 часов (08:00–23:00) случаются ЧП: набеги, бунты, катаклизмы, интриги. Реагируйте кнопками в течение 10 минут, иначе казна уйдёт в минус.\n\n` +
      `Mini App нужен только для удобного визуального управления: основные механики доступны прямо в группе.`,
    reply_markup: {
      inline_keyboard: [[{ text: "🏰 Открыть государство", url: link }]],
    },
  });
}
...
export async function POST(request: Request) {
  ...
  try {
    console.info("WARSTATE_RUNTIME=3.9-stable");
