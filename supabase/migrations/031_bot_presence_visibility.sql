-- WARSTATE v3.11 — hide states whose Telegram group no longer has the bot
-- If WARSTATE bot is removed/kicked from a group chat, that state's island
-- must stop appearing on the shared map and in leaderboards for everyone
-- else, without touching its own data (so re-adding the bot instantly
-- restores full visibility with nothing lost).
begin;

alter table public.states add column if not exists bot_present boolean not null default true;
alter table public.states add column if not exists bot_removed_at timestamptz;

-- Only ever need to look up the (usually tiny) set of currently-hidden states.
create index if not exists states_bot_absent_idx on public.states(id) where bot_present = false;

-- gw_get_islands is the primary read path for the world map. Filtering here
-- is the single source of truth: every caller (map, radar, world-fit) goes
-- through it, so a kicked state disappears from the map in one place.
...
as $$
  with ranked as (
    ...
    where coalesce(s.bot_present, true) = true
  ), nearby as (
